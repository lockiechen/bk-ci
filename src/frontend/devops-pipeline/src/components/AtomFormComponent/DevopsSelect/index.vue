<template>
    <div class="select-input" v-bk-clickoutside="handleBlur">
        <input ref="inputArea" class="bk-form-input" type="text" v-bind="restProps" v-model="displayName"
            :disabled="disabled || loading" :title="displayName" autocomplete="off" @focus="handleFocus"
            @keypress.enter.prevent="handleEnterOption" @keypress="handleKeyPress" @keydown.up.prevent="handleKeyup"
            @keydown.down.prevent="handleKeydown" @keydown.tab.prevent="handleBlur" />
        <i v-if="loading" class="bk-icon icon-circle-2-1 option-fetching-icon spin-icon" />
        <i v-else-if="!disabled && value" class="bk-icon icon-close-circle-shape option-fetching-icon"
            @click.stop="clearValue" />
        <div class="dropbox-container" v-show="hasOption && optionListVisible && !loading" ref="dropMenu">
            <ul>
                <template v-if="hasGroup">
                    <li v-for="(item, index) in filteredList" :key="item.id + index" :disabled="item.disabled">
                        <div class="option-group-name">{{ item.name }}</div>
                        <div class="option-group-item" v-for="(child, childIndex) in item.children" :key="child.id"
                            :class="{
                                active: child.active,
                                selected: child.selected
                            }" :disabled="child.disabled" @click.stop="selectOption(child)"
                            @mouseover="setSelectGroupPointer(index, childIndex)" :title="item.name">
                            {{ child.name }}
                        </div>
                    </li>
                </template>
                <template v-else>
                    <li v-for="(item, index) in filteredList" :key="item.id + index" :class="{
                        'option-item': true,
                        active: item.active,
                        selected: item.selected
                    }" :title="item.name" :disabled="item.disabled" @click.stop="selectOption(item)"
