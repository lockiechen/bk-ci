...
export const useFlowHomeContentStore = defineStore('flowContentList', () => {
  ...
  const isShowEnableDialog = ref(false);
      ...
      flowAction: [
        {
          ...
          handler: (data: ContentTableItem) => openActionDialog(data, DialogType.ENABLE)
        }, {
          text: t('flow.content.addTo'),
          tooltips: '我是一个tooltips',
          ...
          handler: (data: ContentTableItem) => openActionDialog(data, DialogType.DELETE)
        }
      ]
    };
  }
  ...
  function openActionDialog(data: ContentTableItem, dialogType: DialogType) {
    ...
    // 根据弹窗类型设置对应的显示状态
    switch (dialogType) {
      case DialogType.ENABLE:
        isShowEnableDialog.value = true;
        break;
  ...
  function closeAllDialogs() {
    isShowEnableDialog.value = false;
