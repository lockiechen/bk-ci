...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    function handleKeyChangeWrapper(index: number) {
      return (val: string) => handleKeyChange(index, val)
    }

    function handleValueChangeWrapper(index: number) {
      return (val: string) => handleValueChange(index, val)
    }

    function handleRemoveWrapper(index: number) {
            ...
            <li key={index} class={styles.keyValueItem}>
              <div class={`${styles.itemInput} ${styles.key}`}>
                <Input
                  ...
                  onChange={handleKeyChangeWrapper(index)}
                />
              </div>
              ...
              <div class={`${styles.itemInput} ${styles.value}`}>
                <Input
                  ...
                  onChange={handleValueChangeWrapper(index)}
                />
              </div>
              {!props.disabled && !props.editValueOnly && (
                <div class={styles.itemAction} onClick={handleRemoveWrapper(index)}>
