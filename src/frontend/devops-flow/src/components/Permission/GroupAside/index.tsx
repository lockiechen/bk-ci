...
export default defineComponent({
  ...
  emits: ['choose-group', 'create-group', 'close-manage', 'change-group-detail-tab'],
  ...
  setup(props, { emit }) {
     ...
     */
    const groupCountField = computed(() =>
      getGroupCountFields(props.resourceType),
    );
    ...
    const resourceTypeName = computed(() =>
      getResourceTypeName(props.resourceType, (key) =>
    ...
    const closeManageTips = computed(() =>
      getCloseManageTips(props.resourceType, (key) =>
        t(`flow.permission.${key}`),
      ),
    );
        ...
        if (page.value === 1 && groupList.value.length > 0) {
          ...
          const chooseGroup =
            groupList.value.find(
              (group) => String(group.groupId) === String(queryGroupId),
            ) || groupList.value[0];
    ...
    const handleChooseGroup = (group: GroupInfo) => {
      ...
      curGroupIndex.value = groupList.value.findIndex(
        (item) => item.groupId === group.groupId,
      );
      emit('choose-group', group);
    };
      ...
      switch (messageData.code) {
        case 'create_user_group_submit':
          refreshList().then(() => {
            const group =
              groupList.value.find(
                (g) => g.groupId === messageData?.data?.id,
              ) || groupList.value[0];
            if (group) handleChooseGroup(group);
          });
          break;
        ...
        case 'add_template_confirm':
          if (currentGroup && messageData.data) {
            currentGroup.departmentCount +=
              messageData.data.departments?.length || 0;
        ...
        case 'remove_template_confirm':
          if (currentGroup && messageData.data?.members) {
            const departments = messageData.data.members.filter(
              (i) => i.type === 'department',
            );
            const users = messageData.data.members.filter((i) => i.type === 'user');
            const templates = messageData.data.members.filter(
              (i) => i.type === 'template',
            );
        ...
        case 'change_group_detail_tab':
          if (messageData.data?.tab) {
            ...
            emit('change-group-detail-tab', messageData.data.tab);
          }
          break;
      ...
      >
        ...
        <div class={styles.groupList}>
          {groupList.value.map((group) => (
            <div
              ...
              class={[
                styles.groupItem,
                activeTab.value === group.groupId && styles.groupActive,
              ]}
              ...
              <div class={styles.numContainer}>
                {groupCountField.value.map((item) => (
                  <div key={item} class={styles.numBox}>
                    ...
                    />
                    <div class={styles.groupNum}>
                      {group[item as keyof GroupInfo]}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {props.showCreateGroup && (
          <div
            class={[
              styles.groupItem,
              activeTab.value === '' && styles.groupActive,
            ]}
        ...
        >
          ...
          <div class={styles.closeTips}>
            ...
            <p>
              ...
              {closeManageTips.value ||
                t('flow.permission.removeEditorsExecutorsViewers')}
            </p>
            <p>
              <i class={styles.warningIcon}>⚠</i>
          ...
          <div class={styles.confirmClose}>
            <span class={styles.confirmText}>
              {t('flow.permission.cannotRecoverWarning')}
            </span>
          </div>
